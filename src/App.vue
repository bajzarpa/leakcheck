  ...
  <div id="app">
    <div class="min-w-screen min-h-screen bg-gray-800 flex items-center justify-center px-5 py-5">
      <div class="w-full mx-auto rounded-xl bg-gray-100 shadow-lg p-10 text-gray-800 relative overflow-hidden min-w-80 max-w-3xl">
        ...
        <div
          v-if="isSubmitted && !isHit"
          class="py-3 px-2 mt-4 mb-0 bg-green-300 text-green-800 rounded border border-green-600"
        >Nem szerepel</div>
        <div
          v-if="isSubmitted && isHit"
          class="py-3 px-2 my-2 bg-red-300 text-red-800 rounded border border-red-600 mt-4"><strong>Szerepel!</strong> Mi a teendő? Érdemes jelszót cserélj minden általad fontosabbnak tartott oldalon, ügyelj rá, hogy megfelelő erősségű jelszót használj és ahol lehet a kétlépcsős azonosítást használd!</div>

        <p class="mt-4"><small>
          <em>Disclaimer:</em> ez az adatbázis az interneten fellelhető,
          ...
          Az oldal kizárólagos célja, hogy a felhasználóknak lehetősége legyen ellenőrizni érintettek-e az adatszivárgásban.</small></p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async check () {
      ...
      const phone =  users.where('phone', '==', sha256(this.queryText)).get()
      const id = users.where('id', '==', sha256(this.queryText)).get()
